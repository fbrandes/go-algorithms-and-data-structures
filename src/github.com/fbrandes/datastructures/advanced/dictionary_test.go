...
func populateDictionary(count int, start int) *Dictionary {
	...
	for i := start; i < (start + count); i++ {
		dict.Insert(fmt.Sprintf("key%d", i), fmt.Sprintf("value%d", i))
	}
	return &dict
}
...
func TestInsert(t *testing.T) {
	...
	dict.Insert("key1", "value1") //should not add a new one, just change the existing one
	if size := dict.Size(); size != 3 {
		t.Errorf("wrong count, expected 3 and got %d", size)
	}
	dict.Insert("key4", "value4") //should add it
